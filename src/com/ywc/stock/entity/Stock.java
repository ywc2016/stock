...
public class Stock {
    ...
    public double getPriceByDate(Date date) {
        for (int i = 0; i < this.getDateArrayList().size(); i++) {
            if (this.getDateArrayList().get(i).equals(date)) {
                return this.getClosePriceArrayList().get(i);
            }
        }
        System.out.println("编号为 " + this.getName() + " 的股票没有找到日期为 " + date + " 的收盘价格.");
        return 0;
    }
}
